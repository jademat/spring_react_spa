...
const BoardList = () => {
    ...
    // useRef : 특정 DOM 요소에 접근하기 위해 사용하는 리액트 hook
    const ftypeRef = useRef(null);
    const fkeyRef = useRef(null);

    // 엔드포인트에서 path 변수 추출
    // useParam : URL 경로상의 정의된 매개변수로 값을 추출
    ...
    const pglink = (fkey === null) ?
        `/board/list/` :
        `/board/find/${ftype}/${fkey}/`;

    const fetchURL = `http://localhost:8080/api${pglink}${cpg}`;


    // react에서 부수작업 side effect을 수행하기 위한 hook
                    ...
                    <td colSpan="3" className="text-start">
                        <div className="d-flex align-items-center gap-3">
                            <select className="form-select" style={{width: "120px"}} id="findtype" name="findtype"
                                    defaultValue="title" ref={ftypeRef} >
                                <option value="title">제목</option>
                                <option value="userid">작성자</option>
                                <option value="contents">내용</option>
                                <option value="titconts">제목&내용</option>
                            </select>
                            <input className="form-control" style={{width: "250px"}} id="findkey" name="findkey"
                                   ref={fkeyRef}/>
                            <button className="btn btn-success" id="findbtn" onClick={goBoardFind}><i
