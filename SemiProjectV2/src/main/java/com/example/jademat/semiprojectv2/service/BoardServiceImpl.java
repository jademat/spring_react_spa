...
import com.example.jademat.semiprojectv2.domain.*;
import com.example.jademat.semiprojectv2.repository.BoardRepository;
import com.example.jademat.semiprojectv2.repository.ReplyRepository;
...
public class BoardServiceImpl implements BoardService {
    ...
    public BoardListDTO findBoard(int cpg, String findtype, String findkey) {
        ...
        switch (findtype) {
            case "title":
                pageboards = boardRepository.findByTitleContains(pageable, findkey);
                break;
            case "contents":
                pageboards = boardRepository.findByContentsContains(pageable, findkey);
                break;
            case "userid":
                pageboards = boardRepository.findByUseridContains(pageable, findkey);
                break;
            case "titconts":
                pageboards = boardRepository.findByTitleContainsOrContentsContains(pageable, findkey, findkey);
                break;
